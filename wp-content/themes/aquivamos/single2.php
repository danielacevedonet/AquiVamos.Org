...
<html lang="es">
  <head><?php include('prueba.php'); ?>
    ...
    <style><?php include('css/single.php'); ?></style>
  </head>
  <body>
    <div class="Content">
      ...
      <div class="Breadcrumb-content"><?php if (function_exists('dimox_breadcrumbs')) dimox_breadcrumbs(); ?></div>
          ...
          <article class="Section">
            <header class="Section-header">
              <div class="Section-article">
                <h2 title="<?php the_title(); ?>" class="Section-title"><?php the_title(); ?></h2><span class="Section-autor">de <?php the_author(); ?> - <?php the_time('d-m-y'); ?></span><span class="Section-vistas"></span>
              </div>
            </header>
          </article>
          <div class="Section-redes">
            <h3 class="Section-redesTitle"><strong>Compartir</strong></h3>
            <div class="Redes"><a href="https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink(); ?>" title="Comparteme En Facebook" target="_blank" rel="nofollow" data-social_name="facebook" data-social_type="share" data-location="inline" class="Redes-facebook">Facebook</a><a href="https://twitter.com/intent/tweet?url=<?php the_permalink(); ?>&text=<?php the_title_attribute(); ?>%20por%20@aquivamosorg" title="Comparteme en Twitter" target="_blank" rel="nofollow" data-social_name="twitter" data-social_type="share" data-location="inline" class="Redes-twitter">Twitter</a><a href="https://plus.google.com/share?url=<?php the_permalink(); ?>" title="Comparteme en Google +" target="_blank" rel="nofollow" data-social_name="twitter" data-social_type="share" data-location="inline" class="Redes-googleplus">Google +</a></div>
          </div>
          <article class="Section-description">
            <h3 class="Section-descriptionTitle"><strong>Descripción</strong></h3><?php the_content(); ?>
            <footer class="Section-footer"> 
              <div class="Section-categoria"><strong>Categoria:</strong> <?php the_category() ?></div>
              <div class="Section-tag"><?php the_tags(); ?></div>
            </footer>
          </article><?php endwhile; else : ?><p><?php _e( 'Disculpa, No encontramos lo que buscas.' ); ?></p><?php endif; ?>
          <div class="Section-comentario"><?php comments_template(); ?>
          </div>
        </div>
        <div class="Lateral">
          <div class="Lateral-anuncios"><?php include('anuncios/resultados.php'); ?></div>
          <div class="Lateral-paginas"><?php rewind_posts(); ?><?php query_posts('cat=20'); ?><?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
            <article class="Paginas">
              <div class="Paginas-left">
                <figure class="Paginas-img"><?php the_post_thumbnail(array(115,65)); ?></figure>
              </div>
              <div class="Paginas-right">
                <header>
                  <h3 class="Paginas-title"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="Paginas-link"><?php the_title(); ?></a></h3><span class="Paginas-autor"><?php the_author(); ?></span>
                </header>
              </div>
            </article><?php endwhile; else : ?><p><?php _e( 'Disculpa, No encontramos lo que buscas.' ); ?></p><?php endif; ?>   
          </div>
